...
export default function SteamLoginButton({
  ...
  return (
    <a
      href="/api/auth/steam"
      className={`inline-flex items-center justify-center bg-[#FF2E44] hover:bg-[#FF2E44]/80 text-white font-rajdhani font-semibold uppercase tracking-wider rounded-xl transition-all duration-300 hover:shadow-lg hover:shadow-[#FF2E44]/20 ${
        sizeClasses[size]
      } ${fullWidth ? "w-full" : ""} ${className}`}
    >
      <SteamIcon className={size === "sm" ? "h-3 w-3" : size === "lg" ? "h-5 w-5" : "h-4 w-4"} />
      Entrar com Steam
    </a>
  );
}
